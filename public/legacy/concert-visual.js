...
function getViewportSize() {
  return {
    w: Math.max(1, window.innerWidth || 0, document.documentElement.clientWidth || 0),
    h: Math.max(1, window.innerHeight || 0, document.documentElement.clientHeight || 0)
  };
}
...
function syncCanvasSize() {
  ...
  if (width !== w || height !== h) resizeCanvas(w, h, false);
...
function setup() {
  ...
  viewportObserver = new ResizeObserver(scheduleCanvasResize);
  viewportObserver.observe(document.documentElement);
  scheduleCanvasResize();
  setTimeout(scheduleCanvasResize, 120);
}
